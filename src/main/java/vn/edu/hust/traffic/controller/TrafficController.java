...
/**
 * Bộ điều khiển giao thông Quản lý danh sách các phương tiện và cập nhật mô
 * phỏng.
 */
public class TrafficController {
	...
	public void update(double dt) {
		...
		spawnVehiclesAutomatically(dt);
		updateVehicles(dt);
		removeVehiclesOutsideScreen();

		intersection.update();
	}

	private void updateTrafficLights(double dt) {
		if (SimulationConfig.autoMode) {
			phaseController.update(dt);
		}
	}

	private void spawnVehiclesAutomatically(double dt) {
		if (!autoSpawnEnabled) {
			return;
		}

		double spawnInterval = getSpawnInterval();
	...
	private double getSpawnInterval() {
		return switch (SimulationConfig.density) {
		case 1 -> 5.0;
		case 2 -> 3.0;
		case 3 -> 1.5;
		default -> 3.0;
	...
	private void removeVehiclesOutsideScreen() {
		vehicles.removeIf(v -> v.getX() < -200 || v.getX() > WIDTH + 200 || v.getY() < -200 || v.getY() > HEIGHT + 200);
	}
	...
	private void spawnVehicle(int directionIdx) {
		...
		int turnIntention = 0;
		if (turnRand < 2)
			turnIntention = 1; // 20% rẽ trái
		else if (turnRand < 4)
			turnIntention = 2; // 20% rẽ phải

		double offset = 0;
		if (turnIntention == 1)
			offset = LANE_PRIORITY; // Sát tim đường để rẽ trái
		else if (turnIntention == 2)
			offset = LANE_BIKE; // Sát vỉa hè để rẽ phải
		else
			offset = LANE_CAR; // Đi thẳng ở làn giữa
	...
	public void spawnVehicleManually(String typeStr) {
		...
		switch (typeStr) {
		case "Emergency":
			v = new Ambulance("Amb" + vehicleCount, x, y, speed, dir, true);
			break;
		case "Ambulance":
			v = new Ambulance("Amb" + vehicleCount, x, y, speed, dir, false);
			break;
		case "FireTruck":
			v = new FireTruck("Fire" + vehicleCount, x, y, speed, dir);
			break;
	...
	public IntersectionPhaseController getPhaseController() {
